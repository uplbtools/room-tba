...
const GROUPS: Group[] = [
  ...
  { code: "DBVS LR", fullName: "DBVS Lecture Room", aliases: ["DBVS Lecture Room"] },
  {
    code: "CDC RM 201B",
  ...
  },
  {
    ...
    aliases: ["CDC GRAD ROOM", "CDC GRAD RM", "CDC GRADRM", "CDC Graduate Room"],
  },
  ...
  { code: "SDS Lab", fullName: "DSDS SDS Lab", aliases: ["SDS lab", "DSDS SDS Lab"] },
  {
    code: "Malacology Lab",
    fullName: "Malacology Laboratory (BS Snail Room)",
    aliases: ["Malacology Lab (Snail Room)", "Malacology Laboratory (BS Snail Room)"],
  },
  { code: "CFNR Varrons Room", fullName: "Varrons Room", aliases: ["Varrons Room", "AdminBldg_VarronsRm"] },
  {
...
type Room = { id: number; room_code: string; full_name: string | null; version: number };
...
async function repoint(client: pg.PoolClient, keepId: number, dropIds: number[]) {
  if (dropIds.length === 0) return;
  const values = [keepId, dropIds];
  for (const table of ["classes", "final_exams", "organizations"]) {
    await client.query(`UPDATE ${table} SET room_id = $1 WHERE room_id = ANY($2::int[])`, values);
  }
  ...
  if (!keepPosition && positions[0]) {
    await client.query("UPDATE room_positions SET room_id = $1 WHERE id = $2", [keepId, positions[0].id]);
  }
  await client.query("DELETE FROM room_positions WHERE room_id = ANY($1::int[])", [dropIds]);
  await client.query(
...
async function main() {
  ...
  try {
    for (const group of GROUPS) {
      ...
      if (rows.length === 0) throw new Error(`No current room found for ${group.code}`);
      const keep = rows.find((room) => room.room_code.toLowerCase() === group.code.toLowerCase()) ?? rows[0];
      const drop = rows.filter((room) => room.id !== keep.id);
      console.log(`${group.code} <= ${rows.map((room) => `#${room.id} ${room.room_code}`).join(" | ")}`);
      if (!apply) continue;
      ...
      try {
        ...
        const { rows: collisions } = await client.query<{ normalized_alias: string; target_id: number }>(
        ...
        if (collisions.length) throw new Error(`Alias collision for ${group.code}`);
        await repoint(client, keep.id, drop.map((room) => room.id));
        await client.query(
        ...
        if (!rowCount) {
          await client.query(
            ...
            [
              keep.id,
              JSON.stringify(rows),
              JSON.stringify({ roomCode: group.code, fullName: group.fullName, aliases: group.aliases, mergedRoomIds: drop.map((room) => room.id) }),
              keep.version,
              keep.version + 1,
    ...
    if (apply) {
      await pool.query("UPDATE \"update\" SET sync_key = gen_random_uuid() WHERE table_name IN ('rooms', 'classes', 'final_exams', 'organizations')");
