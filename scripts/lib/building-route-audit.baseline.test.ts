...
describe("building route audit baseline", () => {
  test("audits the complete building export without invalid coordinates", async () => {
  ...
  test("legacy audit snapping stays in parity with the legacy nearest-node baseline", async () => {
    const { buildings, graph, report } = await loadBaseline();
