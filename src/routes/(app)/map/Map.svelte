...
const filteredPlaces = $derived.by(() => {
	if (!loaded || !map.showPlaces || placePinFilter === "none") {
		return [];
	}
...
$effect(() => {
	map.poiPinsZoomVisible = poiPinsVisible;
});
...
function handleZoom() {
	...
	zoomLevel = map.getRawInstance().getZoom();
...
$effect(() => {
	...
	if (map.isStyleLoaded()) {
		...
		mapInstance.once("load", applyPalette);
	}
	return () => {
		cancelled = true;
	};
...
$effect(() => {
	...
	if (map.isStyleLoaded()) {
		...
		mapInstance.once("load", applyDimensionLayers);
	}

	mapInstance.on("pitch", applyDimensionLayers);
	mapInstance.on("moveend", applyDimensionLayers);
	return () => {
		cancelled = true;
...
$effect(() => {
	...
	if (map.isStyleLoaded()) {
		...
		mapInstance.once("load", applyBounds);
	}
});
...
$effect(() => {
	if (map.getRawInstance() && !directions) {
		const initDirections = () => {
			if (!directions && map.getRawInstance()) {
				directions = new MapLibreGlDirections(map.getRawInstance(), {
		...
		if (map.getRawInstance().isStyleLoaded()) {
			initDirections();
		} else {
			map.getRawInstance().once("load", initDirections);
		}
	}
});
...
$effect(() => {
	...
	const waypoints = userLocation.routeWaypoints;
	if (waypoints && waypoints.length >= 2) {
	...
	} else if (userLocation.routeOrigin && userLocation.destination) {
		...
		if (map) {
			const bounds = new mapGl.LngLatBounds();
			bounds.extend(userLocation.routeOrigin);
			bounds.extend(userLocation.destination);
			map.fitBounds(bounds, {
...
$effect(() => {
	...
	mapInstance.on("click", handlePinPick);
	return () => {
...
$effect(() => {
	...
	mapInstance.on("click", handlePlacementClick);
	return () => {
...
$effect(() => {
	...
	mapInstance.on("click", handleTravelTimeClick);
	return () => {
...
$effect(() => {
	...
	const accuracyMeters = userLocation.accuracyMeters;
	if (!map) return;
...
// "My classes" highlight (#see map.highlightMyBuildings): emphasize
...
const classHighlightActive = $derived(
	map.highlightMyBuildings && activePlannerRoomCodes.length > 0,
);

$effect(() => {
	if (!map.highlightMyBuildings) return;
		...
		{#if mapStyle}
			...
			>
				{#if userLocation.coords}
					<Marker lngLat={userLocation.coords}>
				...
				{#each scheduleRouteStore.dayStops as stop, index (`schedule-stop:${index}:${stop.courseCode}:${stop.roomCode}:${stop.scheduleSlot}`)}
					{#if stop.coords}
						{@const routeActive =
							...
							userLocation.routeWaypoints !== null}
					...
					{#key `${editKey}:${canDragPin(editKey)}`}
						...
						>
							<MapEntityPin
								...
								useCentralHoverPreview={centralHoverPreview}
								{previewSuppressed}
								onpointerenter={(event) => handleBuildingPinPointerEnter(building, editKey, event)}
								onpointerleave={() => handleBuildingPinPointerLeave(editKey)}
							>
								<PinGlyph name="building" size={20} />
							</MapEntityPin>
					...
					<Marker lngLat={[lon, lat]}>
						<MapEntityPin
							...
							labelVisible={(isStudentOrganization(org.category)
								? zoomLevel >= 19.5
								: zoomLevel >= 18.5) || activeOrgName === org.name}
							useCentralHoverPreview={centralHoverPreview}
							{previewSuppressed}
							onclick={() => handleOrgMarkerClick(org.name, org.id)}
							onpointerenter={(event) => handleOrganizationPinPointerEnter(org, event)}
...
<!-- {#if eventPlacementStore.active}
	...
	{:else}
		<div bind:this={editChromeEl} class="event-placement-toolbar" role="status" aria-live="polite">
			<div class="event-placement-copy">
				<strong>Choose event location on the map</strong>
				<span>
					{eventPlacementStore.creating
