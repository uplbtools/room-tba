import { PREVIEW_BASE, integrationPassword } from "../helpers/env";
...
export async function patchBuilding(
  ...
  return fetch(`${PREVIEW_BASE}/api/admin/buildings/${id}`, {
    method: "PATCH",
    headers: {
...
export async function submitProposal(
  ...
  return fetch(`${PREVIEW_BASE}/api/proposals`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
export async function approveProposalHttp(id: number, cookie: string) {
  return fetch(`${PREVIEW_BASE}/api/admin/proposals/${id}/approve`, {
    method: "POST",
