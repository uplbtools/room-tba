...
describeIntegration("editor credits service (#274)", () => {
  ...
  beforeEach(async () => {
    ...
    await client.query(`
      ...
      VALUES
        ('${PREFIX}-editor', 'Direct Editor', '', 'editor', true, true),
        ('${PREFIX}-contributor', 'Approved Contributor', '', 'contributor', true, true),
        ('${PREFIX}-hidden', 'Hidden Person', '', 'editor', true, false),
        ('${PREFIX}-inactive', 'Inactive Person', '', 'editor', false, true)
    `);
    await client.query(
  ...
  test("includes active editor/contributor and legacy records, without private fields", async () => {
    ...
    expect(names).toContain("Niño Anthony Marmeto");
    expect(credits.find((credit) => credit.name === "Rosh Almario")).toEqual({
      name: "Rosh Almario",
